...
class OutlineNode extends TempNode {
	...
	/**
	 * A mask value that represents the visible selected objects. 
	 * 
	 * @return {Node<float>} The visible selected mask.
	 * @reviser lijuhong 添加 visibleSelected 属性，表示可见的选中对象的 mask 值
	*/
	get visibleSelected() {
	...
	setup() {
		...
		const composite = Fn( () => {
			...
			// @reviser lijuhong 修改输出，r 通道承载可见边缘掩码，g 通道承载隐藏边缘掩码，b 通道承载可见选中区域掩码
			const selectedFactor = maskColor.g.oneMinus().greaterThan( 0.0 ).select( 1.0, 0.5 );
			const selectedMask = maskColor.r.oneMinus().mul( selectedFactor );
			return vec4( edgeValue.r.mul( maskColor.r ), edgeValue.g.mul( maskColor.r ), selectedMask, 1.0 );
